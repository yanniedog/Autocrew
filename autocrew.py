...
class AutoCrew:
    ...
    def get_agent_data(self, overall_goal, delimiter):
        ...
        try:
            if self.llm_endpoint == 'ollama' and self.ollama:
                ...
                logging.debug(f"Number of tokens in the response: {self.count_tokens(response)}")
                return response
    ...
    def rank_crews(self, csv_file_paths, overall_goal, verbose=False):
        ...
        concatenated_csv_token_count = self.count_tokens(concatenated_csv_data)
        ...
        logging.debug(f"Number of tokens in the response: {self.count_tokens(ranked_crew)}")
    ...
    def save_ranking_output(self, ranked_crews, overall_goal):
        ...
        with open(file_path, 'w') as file:
            ...
            for crew_name, ranking in ranked_crews:
                writer.writerow([crew_name, ranking])
        logging.info(f"Your crews have been ranked successfully. See here for details: {file_path}")  # Log the full path of the saved ranking CSV
...
def main():
    ...
    # Read the on_screen_logging_level from the config file
    on_screen_logging_level = config.get('MISCELLANEOUS', 'on_screen_logging_level', fallback='INFO')

    # Initialize logging with the verbosity level from the config file
    initialize_logging(verbose=args.verbose, on_screen_logging_level=on_screen_logging_level)
