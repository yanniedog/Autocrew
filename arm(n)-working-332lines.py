...
def define_agent(agent, search_tool):
    role_var = agent['role'].replace(' ', '').replace('-', '').replace('.', '_')
...
def define_task(agent):
    role_var = agent['role'].replace(' ', '').replace('-', '').replace('.', '')
    return (
        f'task{role_var} = Task(\n'
...
def main():
    ...
    parser.add_argument('-m', '--multiple', type=int, help='Create multiple CrewAI scripts for the same overall goal')
    ...
    try:
        ...
        for i in range(num_scripts):
            ...
            if args.auto_run:
                print(f'\nRunning script {i+1}...')
                os.system(f'python {crewai_script_path}')
