...
class AutoCrew():    
    ...
    def cleanup_files_for_crew(self, crew_name):
        """
        Deletes CSV and script files associated with a specific crew name.

        Args:
            crew_name (str): The name of the crew for which files should be deleted.
        """
        # Define the directory where the scripts are stored
        script_directory = "scripts"
        # Define the pattern for CSV and script files associated with the crew
        csv_pattern = f"crewai-autocrew-*-*-{crew_name}.csv"
        script_pattern = f"crewai-autocrew-*-*-{crew_name}.py"

        # Find and delete CSV files
        for csv_file in glob.glob(os.path.join(script_directory, csv_pattern)):
            try:
                os.remove(csv_file)
        ...
        for script_file in glob.glob(os.path.join(script_directory, script_pattern)):
            try:
                os.remove(script_file)
                logging.info(f"Deleted script file: {script_file}")
            except OSError as e:
                logging.error(f"Error deleting script file {script_file}: {e}")
